...
export default async function FletteHjertekurvSection({
  headingId,
  lang,
}: HeadingIdProps) {
  const { title, ingress, steps, step1 } = (await getDictionary(lang)).pages
    .howToCreateHjertekurvPage.fletteHjertekurvSection;
      ...
      <OrderedListWithIllustrationWrapper>
        ...
        >
          <ReplacePlaceholder
            ...
            component={
              <Popover
                title={step1.popoverTitle}
                content={<>{step1.popoverContent}</>}
              >
                {step1.popoverText}
              </Popover>
            }
